...
helpers do
  ...
  def querySteamCache(steamid) 
    ...
    audit_records = Audit.all(:steamid => steamid, :order => [ :create_datetime.desc ])
    json_obj = nil  
  ...
  def queryProfileCache(steamid)
    min_date = DateTime.now - Rational(8, 24)
    audit_records = AuditProfile.all(:steamid => steamid, :order => [ :create_datetime.desc ])
    json_obj = nil  
    if (audit_records && audit_records.length > 0 && audit_records.first[:create_datetime] > min_date)
      audit_recomm = audit_records.first
      json_obj = audit_recomm[:json]
  ...
  def getSteamDetails(steamid)
    ...
    logger.info { "query steam for #{steamid}" }
    ...
    unless success
      logger.error { "steam conneciton failure" }
      return nil
    end
